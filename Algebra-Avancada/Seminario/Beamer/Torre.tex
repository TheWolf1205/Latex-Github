...
\begin{frame}{}
  ...
  \begin{block}{Torre de automorfismos}
    ...
    \end{align*}
    com $\alpha$ ordinal.\\
    Esta cadeia ascendente é chamada a \textbf{torre de automorfismos}.\\
    Se $\gamma$ é um ordinal limite, definimos $G_{\gamma}=\bigcup_{\beta<\gamma}G_{\beta}$. 
  \end{block}
\end{frame}
\begin{frame}{}
  \begin{block}{Teorema de Wielandt}
    Dado $G$ grupo finito com $Z(G)=\{1\}$ temos que existe $N\in\mathbb{Z}^{+}$ tal que $G_{N}=G_{N+i}$ com $i\in\mathbb{Z}^{+}$. Isto é, que a torre de automorfismos de $G$ estabiliza num número finito de passos. 
  \end{block}
...
\begin{block}{Observação}
    ...
    \end{align*}
    Pelo que podemos assegurar que a cadeia estabiliza num grupo \textbf{completo}.
  \end{block}    
\end{frame}
