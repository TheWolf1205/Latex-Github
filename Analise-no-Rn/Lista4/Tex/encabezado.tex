...
\NewDocumentEnvironment{homeworkProblem}{O{} O{}}
{
    \def\customnumber{#1}
    
    % 🔥 DEFINE PRIMERO
    \ifx\customnumber\empty
        \def\currentProblemName{\arabic{homeworkProblemCounter}}
    \else
        \def\currentProblemName{#1}
    \fi

    % 🔥 LUEGO USA
    \section{Exercício \currentProblemName \ifx&#2&\else (#2)\fi:}

    \nobreak\extramarks{Exercício \currentProblemName}{}\nobreak
...
\newcommand{\hmwkTitle}{Lista 4}
\newcommand{\hmwkDueDate}{\today}
\newcommand{\hmwkClass}{Análise no $\mathbb{R}^{n}$}
\newcommand{\hmwkUniversity}{Universidade Federal de Minais Gerais}
\newcommand{\hmwkAuthorName}{Andrés David Cadena Simons \and acadenas@ufmg.br}
\newcommand{\hmwkInstructor}{Gabriel Aléxis Rondón Vielma}
...
\newtcolorbox[auto counter]{note}[2][]{%
  colback=mygrayback, colframe=mygrayframe, fonttitle=\bfseries,
...
% Redefinir el ambiente de Definition
\newtcolorbox[auto counter]{definition}[2][]{%
  colback=mygrayback, colframe=mygrayframe, fonttitle=\bfseries,
  title=Definição~\thetcbcounter: #2,#1, breakable}
