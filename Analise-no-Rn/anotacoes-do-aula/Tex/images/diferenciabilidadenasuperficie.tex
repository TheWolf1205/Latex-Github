...
\node at (0.9,1.3) {$V$};
\fill (0.6,1) circle (1pt);
  \node[below] at (0.6,1) {$p$}; 
\node at (2.4,2) {$M\subseteq \mathbb{R}^{n}$};
\draw[->, thick] (0.8,-0.3) -- (-1.2,-1.5) node[midway, above] {$f$};
\end{scope}

  \draw[-] (3,-2.4) -- (6,-2.4) node[right] {$\mathbb{R}^{r}$};
  \draw[-] (3.4,-0.4) -- (6.4,-0.4);
  \draw[-] (6,-2.4) -- (6.4,-0.4);
  \draw[-] (3,-2.4) -- (3.4,-0.4);
\fill (4.5,-1) circle (1pt);
  \node[below] at (4.5,-1) {$f(p)$}; 
\draw[->, thick] (0.5,-0.3) -- (2.8,-1.5) node[midway, above] {$f\circ \phi$};
\end{tikzpicture}
